...
- **网页版**：用浏览器打开 `web/index.html` 就有一个实时转换的界面（也可以直接
  用在线链接），预览加 Markdown 源码，可一键复制或下载成 `.md` 文件。页面上的
  转换逻辑不是用 JavaScript 重写的，而是同一份 MoonBit 代码编译成 JS 跑在浏览器里
...
`web/index.html` 是一个交互界面：左边贴 JSON，右边实时出表格，下方给出可以直接
拿去用的 **Markdown 源码**，带复制和下载按钮。写错了直接在预览下方显示中文错误
（带行列位置）。页面上方有五个一键示例。

![错误提示](web/screenshot-error.png)

产出物就是那段 Markdown，所以除了看预览，页面把文本本身也交到你手上：

| 按钮 | 做什么 |
| --- | --- |
| 复制 | 复制 Markdown 到剪贴板，粘到文档里就能用 |
| 下载 .md | 存成 `table.md` 文件 |

解析失败或者结果是空表格时，源码区为空、两个按钮置灰——一眼能看出「现在没东西
可拿」，而不是点了没反应。源码区也可以手动选中复制，不依赖剪贴板 API。
